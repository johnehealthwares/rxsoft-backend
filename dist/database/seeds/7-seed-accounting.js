...
async function seed() {
    ...
    try {
        ...
        for (const acct of glAccounts) {
            await queryRunner.query(`INSERT INTO gl_accounts (organization_id, account_code, account_name, account_type, allows_reconciliation, is_active)
         VALUES ($1, $2, $3, $4, $5, true)
         ON CONFLICT (organization_id, account_code) DO NOTHING`, [DEFAULT_ORG_ID, acct.code, acct.name, acct.type, acct.reconciles]);
            console.log(`  GL Account: ${acct.code} - ${acct.name}`);
        }
        for (const journal of journals) {
            await queryRunner.query(`INSERT INTO journals (organization_id, code, name, journal_type, is_active)
         VALUES ($1, $2, $3, $4, true)
         ON CONFLICT (organization_id, code) DO NOTHING`, [DEFAULT_ORG_ID, journal.code, journal.name, journal.type]);
...
seed();
//# sourceMappingURL=7-seed-accounting.js.map