...
exports.getAllPermissionCodes = getAllPermissionCodes;
exports.getPermissionCodesByModule = getPermissionCodesByModule;
...
function permissionCode(mod, feature, action) {
    return `${mod.id}:${feature.resource}.${action.name}`;
}
function getAllPermissionCodes() {
    const codes = [];
    for (const mod of exports.MODULE_PERMISSIONS) {
...
function getPermissionCodesByModule(moduleId) {
    const mod = exports.MODULE_PERMISSIONS.find((m) => m.id === moduleId);
    if (!mod)
        return [];
    const codes = [];
    for (const feature of mod.features) {
