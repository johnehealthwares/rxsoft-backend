...
function AsyncSelectField({ value, field, onChange, disabled = false, error, formState, ...props }) {
	...
	const selectQuery = useQuery({
		...
		queryFn: async () => {
			...
			*/
			...
			if (field.searchParam?.staticFilters) field.searchParam.staticFilters.forEach((staticFilter) => {
			...
			if (field.searchParam?.staticParams) Object.entries(field.searchParam.staticParams).forEach(([key, val]) => {
				params[key] = val;
			});
