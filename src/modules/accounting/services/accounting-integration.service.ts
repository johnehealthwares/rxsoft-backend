...
export class AccountingIntegrationService {
  ...
  async recordSale(
    ...
    sale: { id: string; saleNumber: string; totalAmount: number; paidAmount: number },
  ...
  async recordSaleRefund(
    organizationId: string,
    refund: { id: string; saleId: string; totalAmount: number; refundNumber: string },
  ): Promise<void> {
    try {
      const journal = await this.journalRepo.findOne({ where: { organizationId, journalType: 'sale', isActive: true } });
      if (!journal) return;
  ...
  async recordStockAdjustment(
    organizationId: string,
    adjustment: { stockBalanceId: string; deltaQuantity: number; reason: string; averageCost: number },
  ): Promise<void> {
  ...
  async recordStockTransfer(
    organizationId: string,
    transfer: { itemId: string; quantity: number; fromLocationId: string; toLocationId: string },
  ): Promise<void> {
    try {
      const journal = await this.journalRepo.findOne({ where: { organizationId, journalType: 'general', isActive: true } });
      if (!journal) return;
  ...
  private async resolveAccountId(organizationId: string, accountCode: string): Promise<string> {
