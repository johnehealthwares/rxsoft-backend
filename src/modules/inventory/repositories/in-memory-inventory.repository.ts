...
export class InMemoryInventoryRepository implements InventoryRepository {
  private readonly stockBalances = new Map<string, StockBalance>();
  private readonly storeStockLocations = new Map<string, StoreStockLocation>();
  private readonly stockMovements = new Map<string, StockMovement>();

  constructor() {
    const seed = new StockBalance(
      'b1',
      'org1',
      {
        id: 'p1',
  ...
  async listStockMovements(query: StockMovementQuery): Promise<{ items: StockMovement[]; total: number }> {
    let items = [...this.stockMovements.values()].filter(
    ...
    if (query.locationId) {
      items = items.filter(
        (item) => item.fromLocationId === query.locationId || item.toLocationId === query.locationId,
      );
    }
  ...
  async listStoreStockLocations(query: StoreStockLocationQuery): Promise<{ items: StoreStockLocation[]; total: number }> {
    let items = [...this.storeStockLocations.values()].filter(
      (item) => item.organizationId === query.organizationId,
    );
  ...
  async adjustStockByReference(payload: AdjustStockByReferencePayload): Promise<StockBalance> {
    ...
    const balance =
      ...
      new StockBalance(
        ...
        payload.lotId ? { id: payload.lotId, code: payload.lotId } : null,
        0,
        0,
        0,
        payload.reorderMinQty ?? null,
        payload.reorderMaxQty ?? null,
      );
