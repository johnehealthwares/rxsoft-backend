...
const orgItemRepo = { findOne: jest.fn() };
const organisationsProxy = { findById: jest.fn() };
...
const salesRepository = { postExistingSale: jest.fn(), createWithSettlement: jest.fn(), list: jest.fn(), findById: jest.fn(), createRefund: jest.fn(), findLastCreated: jest.fn(), getMetrics: jest.fn() };
...
const service = new OrdersService(
  ...
  organisationsProxy as any,
);

function mockOrder(overrides = {}): any {
  return {
    id: 'order1', orderNumber: 'ORD-123', organizationId: null, stockLocationId: null, origin: 'website',
    customerId: 'cust1', paymentMethod: 'cash', orderStatus: 'pending', notes: null, subtotalAmount: 0,
    totalAmount: 0, createdBy: 'u1', createdAt: new Date(), items: [], delivery: null, sale: null, saleId: null,
    ...overrides,
  };
}
...
describe('OrdersService', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    organisationsProxy.findById.mockResolvedValue({ id: 'org1' });
    itemRepo.findOne.mockResolvedValue({ id: 'item1' });
    orgItemRepo.findOne.mockResolvedValue({ id: 'oi-default', itemId: 'item1', isActive: true });
    pricingService.getPricesForItems.mockResolvedValue(new Map());
  });
  ...
  describe('reconcileItem', () => {
    it('links order item to a catalog item and preserves freetext', async () => {
      orderRepo.findOne.mockResolvedValue(mockOrder());
      itemRepo.findOne.mockResolvedValue({ id: 'item42' });
      const orderItem: any = { id: 'oi1', itemId: null, freetextName: 'Custom', quantity: 1 };
      ...
      expect(orderItem.freetextName).toBe('Custom');
      expect(orderItem.resolvedAt).toBeInstanceOf(Date);
    ...
    it('rejects an item that is not active for the org', async () => {
      orderRepo.findOne.mockResolvedValue(mockOrder({ organizationId: 'org1' }));
      itemRepo.findOne.mockResolvedValue({ id: 'item42' });
      orgItemRepo.findOne.mockResolvedValue(null);

      await expect(
        service.reconcileItem('order1', 'oi1', 'item42', { organizationId: 'org1' }),
  ...
  describe('reconcileAllItems', () => {
    it('reconciles all freetext order items', async () => {
      ...
      expect(results[0].freetextName).toBe('A');
  ...
  describe('postOrderAsSale', () => {
    ...
    const confirmedOrder = {
      id: 'order1', orderNumber: 'ORD-123', organizationId: null, stockLocationId: null, origin: 'website',
      customerId: 'cust1', paymentMethod: 'cash', orderStatus: 'confirmed', notes: null, subtotalAmount: 100,
      totalAmount: 100, createdBy: 'u1', createdAt: new Date(), sale: null, saleId: null, delivery: null,
    ...
    it('throws when stock location does not belong to the organisation', async () => {
  ...
  describe('completeSale', () => {
    ...
    it('posts draft sale through the shared sales repository', async () => {
      salesRepository.postExistingSale.mockResolvedValue({ id: 'sale1', status: 'posted' });
      orderRepo.update.mockResolvedValue({} as any);

      const result = await service.completeSale('sale1', currentUser);

      expect(result.status).toBe('posted');
      expect(salesRepository.postExistingSale).toHaveBeenCalledWith('org1', 'sale1', null, 'u1');
      expect(orderRepo.update).toHaveBeenCalledWith({ saleId: 'sale1' }, { orderStatus: 'dispatched' });
    });

    it('propagates posting errors', async () => {
      salesRepository.postExistingSale.mockRejectedValue(new NotFoundException('Draft sale not found'));
      orderRepo.update.mockResolvedValue({} as any);
