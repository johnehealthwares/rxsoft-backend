...
interface HealthConcernDefinition {
  name: string;
  slug: string;
  description: string;
  content: string;
...
const healthConcerns: HealthConcernDefinition[] = [
  {
    name: 'Malaria',
    slug: 'malaria',
...
export async function seedHealthConcerns(manager: EntityManager): Promise<void> {
  
  const repo = manager.getRepository(HealthConcernOrmEntity);
  ...
  const entities = healthConcerns.map((hc) => {
    const entity = new HealthConcernOrmEntity();
    entity.name = hc.name;
    entity.slug = hc.slug;
